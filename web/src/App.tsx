...
function App() {
  ...
  // Преобразуем в Blob (в JPEG)
  const handleDownload = (ctx: HTMLCanvasElement | null) => {
    if (!ctx) {
      console.error("Ссылка на элемент canvas имеет значение null...");
      return;
    }
    ctx.toBlob((blob: Blob | null) => {
      if (!blob) {
      ...
      <div className="controls">
        ...
        {imageData && (
          <>
            ...
            <button onClick={() => handleDownload(canvasRef.current)}>Скачать</button>
          </>
        )}
      </div>
